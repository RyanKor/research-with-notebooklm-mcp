...
등록 후 Claude Code를 새로 시작하면 39개의 NotebookLM 도구를 자연어로 사용할 수 있습니다.
...
### 5. 소스 일괄 추가 (Batch Add)

```
사용자: "~/Documents/papers/ 폴더에 있는 PDF 전부 노트북에 올려줘"

Claude Code:
  → source_add_batch(
      nb_id,
      directory_path="~/Documents/papers/",
      file_pattern="*.pdf"
    )
  → 모든 PDF를 병렬로 업로드 → 성공/실패 요약 반환
```

URL과 파일을 동시에 추가할 수도 있습니다:

```
사용자: "이 3개 URL이랑 로컬 PDF 2개를 한번에 추가해줘"
...
```
사용자: "소스 A와 소스 B의 방법론 차이를 비교해줘"

Claude Code:
  → source_compare(nb_id, source_ids=["src_a", "src_b"], aspect="methodology")
  → 공통점/차이점/고유 기여/종합 평가 구조로 답변
```

### 7. 대화 이어가기 (Chat History)

```
사용자: "아까 질문한 내용에서 더 자세히 알려줘"

Claude Code:
  → chat_ask(nb_id, "좀 더 구체적인 예시를 들어줘", continue_conversation=True)
  → 이전 대화 맥락을 유지한 채 후속 답변
```
...
```
Claude Code:
  → chat_history(nb_id)          // 최근 대화 이력 조회
  → chat_clear(nb_id)            // 대화 초기화, 새 대화 시작
```

### 8. 비동기 콘텐츠 생성

긴 생성 작업을 블로킹 없이 시작하고 나중에 상태를 확인할 수 있습니다:

```
사용자: "팟캐스트 생성 시작만 해놓고, 그 사이에 리포트 먼저 만들어줘"

Claude Code:
  → generate_audio(nb_id, wait=False)        // task_id 즉시 반환
  → generate_report(nb_id, wait=True)        // 리포트는 완료까지 대기
  → generation_status(nb_id, audio_task_id)  // 팟캐스트 진행 상태 확인
  → download_artifact(nb_id, "audio")        // 완료 후 다운로드
```
...
```
Claude Code:
  → auth_check()      // 현재 인증 상태 확인
  → auth_refresh()    // 토큰 강제 갱신 (인증 문제 시)
```
...
Claude Code:
  → notebook_search("ML")
  → 또는: notebook_list(query="ML", sort_by="title", limit=5)
```
...
| `notebook_list` | 전체 노트북 목록 (query/sort_by/limit 필터 지원) |
| `notebook_search` | 제목으로 노트북 검색 (notebook_list의 편의 별칭) |
...
| Tool | 설명 |
|------|------|
| `chat_ask` | 소스 기반 질문 (인용 포함 답변, 대화 이어가기 지원) |
| `chat_ask_specific_sources` | 특정 소스만 대상으로 질문 (대화 이어가기 지원) |
| `chat_configure` | 채팅 페르소나/시스템 프롬프트 설정 |
| `chat_history` | 현재 대화 이력 조회 |
| `chat_clear` | 활성 대화 초기화 (새 대화 시작) |

### 인증

| Tool | 설명 |
|------|------|
| `auth_check` | 인증 상태 확인 (경량 API 호출로 검증) |
| `auth_refresh` | 인증 토큰 강제 갱신 |
...
| Tool | 설명 |
|------|------|
| `generate_report` | 브리핑 문서, 스터디 가이드, 블로그 포스트 생성 (`wait=False`로 비동기 가능) |
| `generate_audio` | 팟캐스트/오디오 오버뷰 생성 (`wait=False`로 비동기 가능) |
| `generate_video` | 비디오 오버뷰 생성 (`wait=False`로 비동기 가능) |
| `generate_quiz` | 퀴즈 생성 (`wait=False`로 비동기 가능) |
| `generate_flashcards` | 플래시카드 생성 (`wait=False`로 비동기 가능) |
| `generate_slides` | 슬라이드 덱 생성 (`wait=False`로 비동기 가능) |
| `generate_infographic` | 인포그래픽 생성 (`wait=False`로 비동기 가능) |
| `generate_mindmap` | 마인드맵 생성 (JSON 구조, 즉시 반환) |
| `generate_data_table` | 데이터 테이블 생성 (`wait=False`로 비동기 가능) |
| `generation_status` | 비동기 생성 작업의 진행 상태 확인 |
...
└── src/notebooklm_mcp/
    ...
    ├── tools/
    │   ├── notebook.py              # 노트북 CRUD + 검색/필터
    │   ├── source.py                # 소스 관리 + 배치 추가
    │   ├── chat.py                  # RAG Q&A + 대화 이력 관리
    │   ├── research.py              # 웹/드라이브 리서치
    │   ├── generate.py              # 콘텐츠 생성 + 비동기 지원
...
│                                             │
│  39 Tools + 4 Prompts + 2 Resources         │
│                                             │
│  ┌─────────────────────────────────────┐    │
...
> 기본적으로 MCP 서버가 완료까지 자동으로 대기합니다(timeout 포함).
> `wait=False`를 사용하면 task_id만 즉시 반환받고, `generation_status`로 진행 상태를 확인할 수 있습니다.
...
# Claude Code에서 인증 확인 (MCP 도구)
# → auth_check()로 인증 상태 확인
# → auth_refresh()로 토큰 갱신
